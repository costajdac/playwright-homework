...
test.describe("Web Table", () => {
  ...
  test("Validate Search by last name", async ({ page }) => {
    ...
    for (let searchItem of searchLastNames) {
      ...
      const listOfOwners = page.locator("tbody tr", {has: page.locator(".ownerFullName")});
    
      if (searchItem === 'Playwright') {
        ...
        for (let eachOwnerRow of await listOfOwners.all()) {
          await expect(eachOwnerRow.locator(".ownerFullName")).toContainText(searchItem);
        }
      }
    }
  });
  ...
  test("Validate phone number and pet name on the owner information page", async ({
    ...
    await targetOwnerTableRow.getByRole('link').click()
    const telephoneColumn = page.locator('tr', {has: page.locator('th', { hasText: 'Telephone' })})
    await expect(telephoneColumn.locator('td')).toHaveText('6085552765')
...
test.describe("Validate Veterinarians Page", () => {
  ...
  test("Validate Specialty Lists", async ({ page }) => {
    ...
    const specialtiesInputFields = page.locator('input[name="spec_name"]');

    for (let input of await specialtiesInputFields.all()) {
