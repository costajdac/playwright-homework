...
test.describe("Web Table", () => {
  ...
  test("Validate the pet name city of the owner", async ({ page }) => {
    ...
    const ownerCityCell = page
      .getByRole("row", { name: "Jeff Black" })
      .locator("td")
      .nth(2);
    await expect(ownerCityCell).toHaveText("Monona");
    const petNameCell = page
      .getByRole("row", { name: "Jeff Black" })
      .locator("td")
      .last();
  ...
  test("Validate Search by last name", async ({ page }) => {
    ...
    const searchLastNames = ["Black", "Davis", "Es", "Playwright"];

    for (let searchItem of searchLastNames) {
      await lastNameSearchInputField.clear();
      await lastNameSearchInputField.fill(searchItem);
      await page.getByRole("button", { name: "Find Owner" }).click();
      await page.waitForTimeout(1000);
      const listOfOwners = page.locator("tbody tr").filter({has: page.locator(".ownerFullName"),});
      const ownersCount = await listOfOwners.count();

      if (ownersCount === 0) {
        await expect(page.getByText(`No owners with LastName starting with "${searchItem}"`)).toBeVisible();
      } else {
        for (let eachOwnerRow of await listOfOwners.all()) {
          const cellValue = await eachOwnerRow.locator(".ownerFullName").textContent();
          expect(cellValue).toContain(searchItem);
        }
      }
    }
  });
  ...
  test("Validate phone number and pet name on the owner information page", async ({
    ...
    const ownersPetNameFromTable = await targetOwnerTableRow
      .locator("td")
      .last()
      .innerText();

    // 3. On the Owner Information page, add the assertion that "Telephone" value in the Owner Information card is "6085552765"
    const telephoneRow = page.locator("tr", {
      has: page.locator("th", { hasText: "Telephone" }),
    });
    ...
    const firstPetNamePetsAndVisits = page
  ...
  test("validate pets of Madison city", async ({ page }) => {
    ...
    await expect(madisonRows.locator("tr")).toHaveText([
      "Leo",
      "George",
      "Mulligan",
      "Freddy",
...
test.describe("Validate Veterinarians Page", () => {
  test("Validate specialty update", async ({ page }) => {
    ...
    await expect(
      page.getByRole("row", { name: "Rafael Ortega" }).locator("td").nth(1),
    ).toHaveText("surgery");
    ...
    const specialitiesRow = page.locator("tbody tr", {
    ...
    await expect(specialitiesRow.locator('[id="1"]')).toHaveValue(
      "dermatology",
    );
  ...
  test("Validate Specialty Lists", async ({ page }) => {
    ...
    const vetSharonJenkinsRow = page.getByRole("row", {
      name: "Sharon Jenkins",
    });
    ...
    await expect(
      vetSharonJenkinsRow.getByRole("cell", { name: "oncology" }),
    ).toBeVisible();
